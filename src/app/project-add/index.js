...
// window.onresize = removeOnKeyboard([btnAddProjectFirestore, baseMenu]);

// const input = document.querySelectorAll('input, textarea');

// input.forEach(el => el.onfocus = () => {
//   baseMenu.style.bottom = '-100px';
//   btnAddProjectFirestore.style.bottom = '-100px';
// });

// input.forEach(el => el.onblur = () => {
//   baseMenu.style.bottom = null;
//   btnAddProjectFirestore.style.bottom = null;
// });
...
auth.onAuthStateChanged(async user => {
  ...
  try {
    ...
    btnAddProjectFirestore.onclick = () => submitProjectFirestore(businessID);
  }
...
async function submitProjectFirestore(businessID) {
  // const arst = document.getElementById('desc').value;
  // console.log(businessID + " " + arst);

  // try {

  //   const lines = arst.split(/\r?\n/);
  //   console.log(lines);

  //   for (let i in lines) {
  //     const splitup = lines[i].split(/(\s+)/);
  //     let name = splitup[0];
  //     let desc = splitup.slice(2).join('');
  ...
  try {
    if (await checkIfNameExists(businessID, name, 'projects')) throw 'Name already exists';
    ...
    for (let i = 0; i < imageCount; i++) {
      await uploadBytes(ref(storage, `${ businessID }/projects/${ docRef.id }/img_${ i }`), smallImages[i]);
      await uploadBytes(ref(storage, `${ businessID }/projects/${ docRef.id }/tiny_img_${ i }`), tinyImages[i]);
    }
...
//     const query = await db.collection('project').where('name', '==', name).get();
...
//       await uploadImage(smallImages[i], `${businessID}/project/${docRef.id}/img_${i}`);
//       await uploadImage(tinyImages[i], `${businessID}/project/${docRef.id}/tiny_img_${i}`);
//     }
