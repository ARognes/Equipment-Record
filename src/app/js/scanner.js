...
async function fetchBarcode(barcode) {
  ...
  // Fetch from db
  try {
    const query = await getDocs(query(collection(db, 'equipment'), where('barcode', '==', parseInt(barcode))));//db.collection('equipment').where('barcode', '==', parseInt(barcode)).get();
    console.log(query);
...
async function fetchQRCode(QRCode) {
  ...
  try {
    ...
    const equipmentDoc = await getDoc(doc(db, 'equipment', QRCode)); //db.collection('equipment').doc(QRCode).get();
    if (!equipmentDoc) return;
    const equipmentData = equipmentDoc.data();
