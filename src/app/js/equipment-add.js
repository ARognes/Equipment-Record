...
async function submitEquipmentFirestore(businessID) {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(images.length, 5); i++) {
      await uploadImage(smallImages[i], `${ businessID }/equipment/${ equipmentID }/img_${ i }`);
      await uploadImage(tinyImages[i], `${ businessID }/equipment/${ equipmentID }/tiny_img_${ i }`);
    }
    
    function uploadImage(image, storageRef) {
      return new Promise((resolve, reject) => {
        const task = uploadBytes(ref(storage, storageRef), image);
